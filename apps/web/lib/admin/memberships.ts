...
export type MembershipApplicationStatus = "new" | "contacted" | "approved" | "rejected" | "archived";
...
export async function getAdminMembershipApplication(id: string): Promise<AdminMembershipApplication | null> {
  const response = await fetch(`${getApiBase()}/membership/admin/applications/${id}`, {
    headers: buildHeaders(),
    cache: "no-store",
  });
...
export async function listAdminMembershipCategories(): Promise<AdminMembershipCategoryFilter[]> {

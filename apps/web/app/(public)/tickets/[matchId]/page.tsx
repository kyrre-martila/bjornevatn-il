...
export async function generateMetadata({
  ...
  const sale = await measureServerTiming(
    {
      flow: "ticket_detail_load",
      route: `/tickets/${params.matchId}`,
      module: "tickets",
      metadata: { matchId: params.matchId },
    },
    () => getPublicTicketSale(params.matchId),
  );
  if (!sale) return {};
  ...
  return buildMetadata({
    ...
    pageDescription:
      sale.description || `Tickets for ${homeTeam} vs ${awayTeam}`,
...
export default async function TicketPurchasePage({
  ...
  if (searchParams?.orderReference) {
    ...
    const order = await measureServerTiming(
      {
        flow: "order_lookup",
        route: `/tickets/${params.matchId}`,
        module: "tickets",
      ...
      () => getTicketOrderSummary(orderReference, searchParams.orderToken),
    );
